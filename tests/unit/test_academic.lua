...
test.test("equation_group with LaTeX commands preserves backslashes")
result = academic.equation_group("\\begin{align}\n\\frac{d}{dx} f(x) &= \\lim_{h \\to 0} \\frac{f(x+h) - f(x)}{h} \\\\\n&= f'(x)\n\\end{align}")
test.assert_not_nil(result)
test.assert_eq(2, #result.lines)
test.assert_eq("\\frac{d}{dx} f(x) &= \\lim_{h \\to 0} \\frac{f(x+h) - f(x)}{h}", result.lines[1])
test.assert_eq("&= f'(x)", result.lines[2])

test.test("equation_group with nested braces and commands")
result = academic.equation_group("\\begin{gather}\n\\alpha + \\beta \\\\\n\\gamma \\cdot \\delta\n\\end{gather}")
test.assert_not_nil(result)
test.assert_eq(2, #result.lines)
test.assert_eq("\\alpha + \\beta", result.lines[1])
test.assert_eq("\\gamma \\cdot \\delta", result.lines[2])
